...
const ONBOARDING_RELIC_IDS: RelicId[] = [
  ...
  // 칼날 파편 씨앗 2종(기초만) — 칼날 파편 손패는 이미 잠금 없이 기본 드로우 풀에 있으니,
  // 처치/사용만으로 더 얻는다는 단순한 문장으로 빌드 축을 슬쩍 보여 준다. 증폭(연마)·심화
  // 계열은 "아직 안 배운 개념"이라 여기 넣지 않는다.
  'whetstone',     // 적 처치 시 칼날 파편 1장 획득
  'hammer',        // 칼날 손패 사용 시 25% 확률로 칼날 파편 1장 획득
]
