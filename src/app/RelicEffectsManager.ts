...
export class RelicEffectsManager {
  ...
  async applyRelicPurchaseEffect(id: RelicId): Promise<void> {
    const { gameState, boardRenderer, cardSpawner, companion, companionDirector, isGameActive, render, snapshotPlayerResources, playPlayerGainTrails, encounterIntroLineOnce } = this.deps
    ...
    if (isGameActive() && !gameState.isGameOver) {
      ...
      companionDirector.sayEnaBark(relicIntro ?? companion.onBuyRelic(id, getRelicDef(id).rarity), {
        importance: BARK_IMPORTANCE.situation,
        // 감상평이 실제 표시될 때 방금 얻은 유물을 부채꼴 스택에서 같은 공용 맥동으로 짚는다.
        onDisplay: () => boardRenderer.pulseEnaHint({ relicIds: [id] }),
      })
    }
  ...
  applyBloodPriceHpLoss(amount: number): void {
    ...
    const gained = Math.floor(enhancements.bloodPriceHpLossAccum / BLOOD_PRICE_THRESHOLD)
    if (gained <= 0) return
