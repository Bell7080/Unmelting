...
async function runQueuedHandIntent(intent: HandUseIntent, slotIndex: number): Promise<void> {
  ...
  // 첫 커밋부터 마지막 예약 정산까지 손패 DOM을 고정해, 다음 입력 좌표가 하강으로 빗나가지 않게 한다.
  ...
  try {
    ...
    // 묶음이 끝난 것이므로, 그제야 최신 모델 손패를 한 번 렌더해 아래로 정리한다.
    if (!handCommitLocked) {
...
function recordRelicActivation(relicId: RelicId, message: string): void {
  ...
  // 누적형(스택) 유물이 조건을 채워 발동하면 보유 유물 팬에서 해당 카드를 부상시켜 확실히 알린다.
...
/**
 * 오디오 개시 — 부팅 게이트의 Click to Start를 누르는 그 입력이 부른다.
 *
 * 배경음은 여기서 직접 시작하지 않고 `armAutoplay()`에 맡긴다. 이 함수는 pointerdown에서
 * 불리는데 배경음은 **그보다 뒤에 오는 click**에 시작돼야 하기 때문이다 — Howler의 html5
 * 언락이 그때 돌기 때문이고, 그 전에 시작하면 잠긴 오디오 노드를 받아 조용히 무음이 된다
 * (자세한 이유는 `BgmManager.armAutoplay`). 같은 손가락 한 번 안에서 일어나는 일이라
 * 화면상으로는 여전히 "누르는 순간 음악이 켜진다".
 *
 * 효과음은 Web Audio라 그 제약이 없어 이 자리에서 바로 연다.
 */
