...
/**
 * 이벤트 문 소멸(클릭 소비/2턴 만료)과 정화 클러치는 모두 index.ts의 같은 레일 정리
 * 블록(하강·리필 → 재그룹 → 개화 → 폭탄 점화 → 새 문 카운트다운 → 별빛 수집)을 거쳐야 한다.
 * index.ts는 모듈 부수효과(DOM 부트) 때문에 직접 import할 수 없어, 소스 계약 검사로
 * 세 경로의 정리 호출이 누락되지 않았음을 회귀 방지한다.
 */
...
describe('이벤트 문 소멸 레일 정리 계약(index.ts)', () => {
  it('문 진입 소비 경로(handleEventDoorClick)가 공통 정리 블록을 전부 수행한다', () => {
    const body = sliceBetween('async function handleEventDoorClick', 'function applyEventChoice')
  ...
  it('2턴 닫힘 만료 경로(tickFrontEventDoors)가 공통 정리 블록을 전부 수행한다', () => {
    const body = sliceBetween('async function tickFrontEventDoors', 'async function sweepFrontStarlights')
    for (const call of REQUIRED_CLEANUP_CALLS) expect(body).toContain(call)
  })
