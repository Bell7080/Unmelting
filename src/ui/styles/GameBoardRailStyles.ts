...
.rail-next-hints {
  position: absolute;
  left: calc(clamp(10px, 1.6vh, 14px) + 4%);
...
.rail-next-hint--enemy {
  background: linear-gradient(90deg, rgba(168, 58, 58, 0), rgba(168, 58, 58, 0.92) 30%, rgba(90, 24, 24, 0.82) 72%, rgba(90, 24, 24, 0));
  box-shadow: 0 0 8px rgba(168, 58, 58, 0.5), 0 0 18px rgba(168, 58, 58, 0.18);
}

.rail-next-hint--trap {
  background: linear-gradient(90deg, rgba(92, 63, 122, 0), rgba(92, 63, 122, 0.94) 30%, rgba(70, 112, 170, 0.68) 72%, rgba(70, 112, 170, 0));
  box-shadow: 0 0 8px rgba(92, 63, 122, 0.52), 0 0 18px rgba(70, 112, 170, 0.2);
}

.rail-next-hint--treasure {
  background: linear-gradient(90deg, rgba(255, 215, 120, 0), rgba(255, 215, 120, 0.94) 28%, rgba(201, 161, 58, 0.9) 74%, rgba(201, 161, 58, 0));
...
.rail-next-hint--special {
  background: linear-gradient(90deg, rgba(255, 255, 255, 0), rgba(255, 255, 255, 0.96) 30%, rgba(210, 226, 255, 0.82) 74%, rgba(210, 226, 255, 0));
  box-shadow: 0 0 8px rgba(255, 255, 255, 0.48), 0 0 18px rgba(210, 226, 255, 0.22);
}

@keyframes rail-next-hint-breathe {
  0%, 100% { opacity: 0.52; transform: translateY(-1px) scaleX(0.74); filter: brightness(0.94); }
  50% { opacity: 0.86; transform: translateY(0) scaleX(0.9); filter: brightness(1.14); }
}
...
/* 온보딩 필드 만료 카운트다운 뱃지 — 밀랍(frozen-badge)에 얹지 않고 완전 독립된 전용 뱃지.
   (예전엔 좌상단 위치/폰트만 물려받으려 frozen-badge를 같이 달았다가 밀랍 흰 배경이 새어
    특이도 싸움이 났다 → 위치/폰트를 자체 정의해 밀랍 의존을 끊는다.)
...
/* waxWitch 3페이지 후방 대기: 90F 조각사처럼 2×3으로 접혀 후방 두 행을 차지한다. */
...
/* 고를 수 있는 칸: 발광을 얇게 눌러 '가리키는 선'에 가깝게 둔다. 화려하게 만들면
   아홉 칸이 한꺼번에 빛나 오히려 어디를 고를지 안 보인다. 대비는 밝기가 아니라
   '주변을 어둡게' 쪽에서 만든다(아래 .is-cell-targeting). */
.boss-gimmick-grid.is-targeting .boss-gimmick-cell.is-hand-target {
  border-style: solid;
  border-color: rgba(255, 214, 140, 0.5);
  background: rgba(255, 205, 110, 0.05);
  box-shadow: inset 0 0 12px -3px rgba(255, 205, 110, 0.35);
  animation: boss-gimmick-target-pulse 2.4s ease-in-out infinite;
}
.boss-gimmick-grid.is-targeting .boss-gimmick-cell.is-hand-target:hover {
  border-color: rgba(255, 236, 180, 0.92);
  background: rgba(255, 219, 150, 0.14);
  box-shadow: 0 0 18px rgba(255, 215, 130, 0.34), inset 0 0 14px -2px rgba(255, 215, 130, 0.34);
  animation: none;
}
@keyframes boss-gimmick-target-pulse {
  0%, 100% { border-color: rgba(255, 214, 140, 0.38); box-shadow: inset 0 0 10px -4px rgba(255, 205, 110, 0.26); }
  50%      { border-color: rgba(255, 224, 160, 0.62); box-shadow: inset 0 0 14px -3px rgba(255, 205, 110, 0.42); }
}

/* ── 칸 타겟팅 중의 보스 타일: 고를 칸만 남기고 나머지를 전부 뒤로 민다 ──────
   보스 일러스트가 밝으면 그 위의 얇은 칸 테두리는 절대 안 읽힌다. 칸을 더 밝히는
   대신 배경·죽은 칸·보스 UI를 눌러 대비를 만든다. */
.boss-face.is-cell-targeting .boss-face-art {
  filter: brightness(0.34) saturate(0.55);
  transition: filter 0.22s ease;
}
.boss-face.is-cell-targeting .boss-face-overlay {
  opacity: 0.5;
}
/* 이름·HP바·ATK칩·턴 뱃지·굳음 표기는 지금 결정에 필요 없는 정보다. */
.boss-face.is-cell-targeting .boss-face-title-row,
.boss-face.is-cell-targeting .boss-face-stats,
.boss-face.is-cell-targeting .boss-face-badge,
.boss-face.is-cell-targeting .boss-frozen-center-badge {
  opacity: 0.22;
  filter: saturate(0.35);
  transition: opacity 0.22s ease, filter 0.22s ease;
}
/* 이미 깨진 칸은 고를 수 없으니 더 깊이 가라앉힌다. */
.boss-gimmick-grid.is-targeting .boss-gimmick-cell.is-broken {
  opacity: 0.3;
}
/* 성한 칸의 종류 비네트는 타겟팅 중 살짝 눌러, 금빛 조준선이 먼저 읽히게 한다. */
.boss-gimmick-grid.is-targeting .boss-gimmick-cell::before {
  opacity: 0.45;
}
...
/* ── 부위 파괴: 그을림 → 흘러내림 → 꺼진 칸 ──────────────────────────────
   유리가 아니라 **양초가 타들어 가는** 어휘를 쓴다. 이 게임의 다섯 물질은
   촛불·양초·밀랍·불씨·제물이고, 보스는 밀랍 덩어리다. 손상은 금이 가는 게 아니라
   가장자리가 그을고 촛농이 흘러내리는 것으로 읽혀야 한다.
   단계는 data-crack이 정하고, 깨진 칸은 0으로 돌아가 굳은 촛농 자국만 남는다. */
...
.hand-card.is-greed-counted {
  /* hue-rotate로 붉게 물들이려 했더니 빨강(0°)이 마젠타로 돌아가 형광 분홍 테두리가
     ...
     밝아지는 게 아니라 그늘이 진다. */
  filter: brightness(0.84) saturate(0.8) contrast(1.06);
  animation: greed-counted-dread 1.2s ease-in-out infinite;
}
/* 딱딱한 테두리를 두르지 않는다 — 선을 그으면 '선택된 카드' UI로 읽힌다.
   안쪽으로 그늘이 차오르고 바깥으로 붉은 빛이 번지는 **발광**으로만 말한다. */
@keyframes greed-counted-dread {
  0%, 100% { box-shadow: 0 0 10px rgba(150, 26, 10, 0.4),  inset 0 0 20px rgba(96, 14, 6, 0.55); }
  50%      { box-shadow: 0 0 26px rgba(196, 44, 16, 0.72), inset 0 0 34px rgba(150, 26, 8, 0.82); }
}
...
/* 맞은 적은 **체력 수치로 말한다.** 카드 자체를 붉게 물들이거나 크게 부풀리면
   레일 전체가 요란해져 정작 몇이 깎였는지가 묻힌다 — 카드 분위기는 그대로 두고,
   깎이는 숫자만 또렷하게 만든다(색 + 한 칸씩 굴러 내려가는 표기). */
.stat.hp .stat-value.is-hp-rolling {
  display: inline-block;
  animation: enemy-hp-roll-pop 0.5s cubic-bezier(0.2, 0.9, 0.25, 1);
  color: #ff8a6a;
}
@keyframes enemy-hp-roll-pop {
  0%   { transform: scale(1); }
  20%  { transform: scale(1.28); text-shadow: 0 1px 3px rgba(0, 0, 0, 0.9); }
  70%  { transform: scale(1.2); text-shadow: 0 1px 3px rgba(0, 0, 0, 0.9); }
  100% { transform: scale(1); }
}
}
