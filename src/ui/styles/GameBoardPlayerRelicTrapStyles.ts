...
/* 즉사(레시피/즉사 손패) 전용 — 체력을 깎지 않고 바로 제거하는 경로라 일반 피해
   롤링이 뜨지 않는다. 그래서 이 문구가 유일한 사망 신호이니 검붉게, 더 불길하게 띄운다. */
.damage-float--instant-kill {
  color: #ff1f3a;
  font-size: clamp(40px, 6.4vw, 88px);
  letter-spacing: 0.04em;
  text-shadow:
    0 4px 5px rgba(0, 0, 0, 0.96),
    0 0 16px rgba(255, 20, 40, 0.98),
    0 0 34px rgba(160, 0, 30, 0.85),
    0 0 60px rgba(90, 0, 30, 0.7);
  -webkit-text-stroke: 1.6px rgba(30, 0, 8, 0.92);
}
