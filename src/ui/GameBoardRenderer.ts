...
`
  ...
  showWaxFigureEffectChain(enemyName: string, effectLabel: string, shiny: boolean): void {
    ...
    host.style.left = `${rect.left + rect.width / 2}px`
    host.style.top = `${rect.top - 86}px`
    document.body.appendChild(host)
    const anim = host.animate(
      // 클러치 배너와 같은 박자(솟아오름 → 체류 → 흐려지며 상승)를 짧게 줄여 쓴다.
      [
        { opacity: 0, transform: 'translate(-50%, 26px) scale(0.84)', filter: 'blur(0px)' },
        { opacity: 1, transform: 'translate(-50%, 2px) scale(1.06)', filter: 'blur(0px)', offset: 0.12 },
        { opacity: 1, transform: 'translate(-50%, -6px) scale(1.0)', filter: 'blur(0px)', offset: 0.22 },
        { opacity: 1, transform: 'translate(-50%, -10px) scale(1.0)', filter: 'blur(0px)', offset: 0.68 },
        { opacity: 0, transform: 'translate(-50%, -30px) scale(1.02)', filter: 'blur(2.4px)', offset: 1 },
      ],
      { duration: 2900, easing: 'cubic-bezier(0.2, 0.8, 0.25, 1)', fill: 'forwards' }
    )
    anim.onfinish = () => host.remove()
  }
