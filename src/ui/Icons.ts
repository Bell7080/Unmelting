...
export function experienceIcon(): string {
  return svg(
    [
      // 불빛 패널 보석보다 세로가 낮은 완만한 다이아 윤곽.
      stroke('M12 3.9 L 20.4 12 L 12 20.1 L 3.6 12 Z', 1.45),
      // 낮은 크라운 면으로 기존 보석과 구분하면서 같은 플랫 SVG 언어를 유지한다.
      stroke('M3.6 12 L 20.4 12', 0.95),
      stroke('M7.6 12 L 12 3.9 L 16.4 12', 0.95),
      // 가운데 불빛 코어도 완만하게 눌러 별자리 노드와 같은 실루엣으로 쓴다.
      fill('M12 9.8 L 14.4 12 L 12 14.2 L 9.6 12 Z'),
    ].join(''),
  )
}
