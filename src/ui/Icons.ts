...
/**
 * 밀랍상(蠟像) — 봉랍 돔(유리 덮개) 아래 작게 굳은 형상. "봉인해 보존한다"는 뜻을
 * 그대로 실루엣으로 옮겼다 — 손패/유물 아이콘과 헷갈리지 않는 새 어휘가 필요했다.
 */
export function waxFigureIcon(): string {
  return svg(
    fitBox(
      [
        // 안에 갇힌 작은 형상(음각으로 뚫어 돔 유리 안쪽처럼 보이게 한다).
        fill('M12 12.4c1 0 1.7.9 1.7 2v2.6H10.3v-2.6c0-1.1.7-2 1.7-2Z'),
        // 돔(유리 덮개) — 아래는 열려 있고 위는 둥글게.
        stroke('M6.6 17.6V10.2C6.6 6.6 9 3.8 12 3.8s5.4 2.8 5.4 6.4v7.4', 1.6),
        // 받침대.
        fill('M5.2 17.6h13.6v1.8a1 1 0 0 1-1 1H6.2a1 1 0 0 1-1-1v-1.8Z'),
        // 손잡이 꼭지.
        stroke('M12 3.8V2.6', 1.6),
      ].join(''),
      2.6,
      20.4,
    ),
