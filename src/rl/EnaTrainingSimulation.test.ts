...
describe('EnaTrainingSimulation', () => {
  it('딥러닝 입력 벡터와 행동 공간 크기를 고정한다', () => {
    ...
    // 스칼라 30 + 9칸×14 + 손패 10×9 = 246.
    expect(observation.features).toHaveLength(246)
  ...
  it('교사 정책으로 100층 한 호의 국면/손패/보스 판단 학습 샘플을 생성한다', () => {
    ...
    expect(dataset.every((sample) => sample.state.length === 246 && sample.nextState.length === 246)).toBe(true)
