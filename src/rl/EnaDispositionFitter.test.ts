...
describe('BASE_DISPOSITION (동봉된 학습 토대)', () => {
  it('안전 경계 안에 있고, 학습 방향(클러치 강화·예측 완화)을 반영한다', () => {
    const base = BASE_DISPOSITION
    const def = defaultDisposition()
    // 클램프 동등성 — 경계를 벗어나지 않는다.
    expect(base).toEqual(clampDisposition(base))
    // 학습 방향: 클러치/각성은 기본보다 관대, 거미줄 예측은 덜 적극.
    expect(base.clutchStrength).toBeGreaterThan(def.clutchStrength)
    expect(base.awakenChance).toBeGreaterThan(def.awakenChance)
    expect(base.predictBaseChance).toBeLessThan(def.predictBaseChance)
  })
