...
export type EventEffect =
  /**
   * 수치로 끝나는 효과 전부를 담는 **하나의** 종류다. 지정한 항목마다 버튼 → 해당 HUD로
   * 획득 블라스트가 나가므로, 한 선택이 "무엇을 내주고 무엇을 얻는가"를 함께 말할 수 있다.
   * 음수 = 대가. 종류를 축(영구 스탯/자원/손패)마다 쪼개 두면 "방패 +4 · 공격력 +1"처럼
   * 축을 걸치는 선택을 못 적고, 결국 문구와 실제 효과가 어긋난다.
   */
  | {
      kind: 'resource'
      /** 영구 최대 체력 증감. */
  ...
  choices: [
    {
      ...
      effect: { kind: 'resource', maxHealth: -5, damage: 1 },
  ...
  minigame: {
    ...
    offers: [
      ...
      { id: 'shield', label: '방패 조르기', hint: '성공하면 방패, 실패하면 흘린 불빛', aim: 'success',
        onSuccess: { shield: 2 }, onFail: { light: -30 } },
      { id: 'wax', label: '촛농 훔치기', hint: '성공하면 콤보 게이지, 실패하면 소량 피해', aim: 'success',
        onSuccess: { candle: 1 }, onFail: { health: -3 } },
