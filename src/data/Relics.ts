...
export const RELIC_DEFINITIONS: Record<RelicId, RelicDefinition> = {
  ...
  // 씨앗(공급): 주사기(커먼) — 체력 5 잃을 때마다 바늘 손패를 흘려 넣어 제물 축을 굴린다.
  // ★ '체력 N 잃을 때마다'는 **자해 + 받는 피해 모두**를 세는 넓은 축이다(문구가 곧 계약).
  syringe: {
    id: 'syringe',
    name: '주사기',
    rarity: 'common',
  ...
  // 복리 페이오프: 피의 대가(유니크) — 흘린 피가 쌓일수록 공격이 영구히 매서워진다.
  // 넓은 축(자해 + 받는 피해)이라 문턱이 40으로 높다 — 좁은 축 유물들과 호환되는 자리다.
  'blood-price': {
    id: 'blood-price',
    name: '피의 대가',
    rarity: 'unique',
