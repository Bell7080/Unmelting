...
describe('EmberSystem spawn weights', () => {
  it('raises ordinary trap odds while preserving former bomb and spore odds', () => {
    ...
    expect(brightBuckets.webTrap).toBe(170)
    expect(brightBuckets.bombTrap).toBe(40)
    expect(brightBuckets.sporeTrap).toBe(40)
    expect(brightBuckets.flower).toBe(90)
    expect(brightWeights).toEqual({ enemy: 440, trap: 250, treasure: 220, flower: 90 })
  })
})
