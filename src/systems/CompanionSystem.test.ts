...
describe('CompanionSystem', () => {
  ...
  it('필드 소개: 한 번에 한 종류만, 순서는 조우 순서와 무관하게 안정된다', () => {
    const c = new CompanionSystem()
    // 빈 목록 → 발화 없음.
    ...
    // ★ 여러 종류가 와도 **한 종류만** 소개한다. 셋을 묶어 말하면 첫 판 시작과 동시에
    //   긴 설명이 한꺼번에 쏟아진다 — 나머지는 호출부가 미뤘다가 그 칸을 만질 때 짚는다.
    for (let i = 0; i < 40; i++) {
      const line = c.introduceFields(['junk', 'rock', 'bush'])!
      expect(line).toContain('바위')
      expect(line).not.toContain('덤불')
      expect(line).not.toContain('잡동사니')
      expect(line).not.toMatch(/[{}[\]]/)
      expect(line).not.toContain('  ') // 이중 공백 없음.
    }
    // 우선순위 목록에 rock이 없으면 그다음 순서(bush)가 나온다.
    expect(c.introduceFields(['junk', 'bush'])!).toContain('덤불')
  })
