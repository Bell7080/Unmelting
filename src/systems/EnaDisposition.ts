...
//
// ☒ **재피팅을 돌렸고 채택하지 않았다(2026-08, 보스 규칙 정합화 이후).** 같은 실험을 다시
//   하지 않도록 남긴다. λ∈{6,9,12} × 시드 5종을 150세대·80시드로 돌려 후보 15개를 뽑았다.
//   - 선택 시드(200개)에서 이긴 상위 후보 둘(21.89·22.10 vs 이 스냅샷 21.70)은 **선택에 쓰지
//     않은** 2차 검증 300시드에서 오히려 졌다(20.79·20.60 vs 21.32). 선택 단계의 우세는
//     시드 잡음이었다.
//   - 모든 후보가 `predictBaseChance`를 탐색 하한(0.02~0.07)으로 밀어붙였다. λ 패널티는
//     예지 빈도에 값을 매기는데 시뮬은 예지의 값(위협 예고 보급)을 손패 1장으로만 근사해
//     제대로 돌려주지 못하기 때문이다. 그 결과 경험 탭 '예지' 축이 캘리브레이션 대역
//     (성장 완료 0.45~0.55)에서 0.36~0.38로 떨어져, 네 축이 한 덩어리로 뭉개진다.
//   - 이 스냅샷은 **바뀐 세계에서도** 기본값을 크게 앞선다(2차 검증 21.32 vs 19.20).
//     세계가 달라졌다고 토대까지 흔들 이유는 없었다.
//   다음 재피팅 전에 고칠 것: 예지의 값을 시뮬이 실제로 돌려주도록 만들거나(예고 위협
//   회피 보상), `predictBaseChance`를 helpCost에서 빼는 것. 그 전에는 후보가 계속
//   예지를 0으로 밀어붙인다.
//
// ☒ **보스 칸 부가물(함정/보물) 도입 후에도 유지했다(2026-08).** 보스전이 플레이어 체력을
//   더 먹게 됐지만, 판정 규칙이 아니라 비용이 바뀐 것이라 전면 재피팅 대신 held-out
//   300시드로 스냅샷과 기본값을 다시 비교했다: **19.60 vs 18.68** — 바뀐 세계에서도
