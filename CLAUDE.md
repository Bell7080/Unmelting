...
- 상점/유물: 10턴마다 레일 셔터 후 닫힌 레일 위 유물 상점. 셔터가 내려가면 체인은 즉시 초기화한다. 유물 카드 하강은 첫 오픈에만 재생하고, 구매 갱신은 DOM을 제자리 갱신해 무애니메이션으로 처리한다. EXIT 후 카드 퇴장 완료 → 레일 덜컹임 → 셔터 역방향 상승 순서를 지킨다.
