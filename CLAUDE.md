...
- 손패: 드롭, 자동 3장 병합, 타깃형/즉발형 효과, 체인 타임라인, 조합 발동, 촛불 게이지. 새 손패는 게이지 바로 아래에서 획득 순서대로 일정하게 하강하고, 보상 드롭이 모두 착지한 뒤 자동 트리플을 검사한다. 신규 트리플은 병합 직전 원본 카드 id를 저장하고 실제 DOM 복제 레이어 2장이 각자의 이전 위치에서 가장 하단 병합 슬롯으로 합류한 뒤 젤리/사각 버스트로 합성감을 준다. 손패 10장까지는 카드 높이를 먼저 압축하고 겹침은 최대한 피한다.
...
- 상점/유물: 10턴마다 레일 셔터 후 닫힌 레일 위 유물 상점. 셔터가 내려가면 체인은 즉시 초기화하고, 셔터가 닫힌 동안 폭탄 점화 글로우 같은 레일 반복 이펙트만 일시정지한다. 셔터 잠금 상태는 렌더러 상태로 보존해 유물 클릭/구매로 보드가 재렌더되어도 사라지지 않는다. 유물 카드 하강은 첫 오픈에만 재생하고, 구매 갱신은 DOM을 제자리 갱신해 무애니메이션으로 처리한다. EXIT 후 카드 퇴장 완료 → UI 정리 → 레일 덜컹임 → 셔터 역방향 상승 순서를 지킨다.
